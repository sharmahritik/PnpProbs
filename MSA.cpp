...
void MSA::printUsage() {
	cerr
			...
			<< endl
			<< "\tYongtao YE (ytye@cs.hku.hk)."
			<< endl
			<< "*************************************************************************"
			<< endl << "Usage:" << endl
			<< "       pnpprobs [OPTION]... [infile]..." << endl << endl
			<< "Description:" << endl
...
MultiSequence* MSA::npdoAlign( MultiSequence *sequences, const int variance_mean ) { 
    ...
    timeUsed = GetElapsedTime ( startTime );
	if(enableRunningtime)
		cerr << "[Main] Non-Progressive Alignment used " << timeUsed - lastUsed << " seconds." << endl;
	lastUsed = timeUsed;

	DoRefinement(finalAlignment, sparseMatrices, model, distances);
	timeUsed = GetElapsedTime ( startTime );
	if(enableRunningtime)
		cerr << "[Main] Iterative Refinement used " << timeUsed - lastUsed << " seconds." << endl << endl;
...
MultiSequence* MSA::ComputeFinalAlignment(MSAGuideTree*tree,
	...
	double lastUsed = timeUsed;
	if(enableRunningtime)
		cerr << "[Main] Progressive Alignment used " << timeUsed << " seconds." << endl;
